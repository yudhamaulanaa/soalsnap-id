...
describe("validasi pembuatan aktivitas", () => {
  ...
  it("menolak aktivitas tanpa soal", () => {
    expect(buatAktivitasSchema.safeParse({ ...dasar, questions: [] }).success).toBe(false);
  });

  it("menolak template yang tidak dikenal", () => {
    expect(buatAktivitasSchema.safeParse({ ...dasar, template: "teka-teki" }).success).toBe(false);
  });

  it("menolak timer di luar 5–60 detik", () => {
    expect(buatAktivitasSchema.safeParse({ ...dasar, timerDetik: 90 }).success).toBe(false);
    expect(buatAktivitasSchema.safeParse({ ...dasar, timerDetik: 3 }).success).toBe(false);
  });

  it("menolak kategori di luar daftar", () => {
    expect(buatAktivitasSchema.safeParse({ ...dasar, kelas: "Kelas 99" }).success).toBe(false);
    expect(buatAktivitasSchema.safeParse({ ...dasar, mapel: "Sihir" }).success).toBe(false);
  });

  it("membiarkan seluruh field kosong pada penyuntingan sebagian", () => {
    expect(ubahAktivitasSchema.safeParse({ title: "Judul baru" }).success).toBe(true);
...
describe("kontak pembuat", () => {
  it("menerima kontak kosong — memang opsional", () => {
    ...
    expect(kontakSchema.safeParse({ name: "Bu Sinta", email: "" }).success).toBe(true);
  });

  it("menolak email yang jelas salah bentuk", () => {
    expect(kontakSchema.safeParse({ email: "bukan-email" }).success).toBe(false);
  });
});
...
describe("penyimpanan hasil peserta", () => {
  ...
  it("menolak mode yang tidak dikenal", () => {
    expect(sesiSchema.safeParse({ score: 1, total: 5, mode: "catur" }).success).toBe(false);
  });
});
